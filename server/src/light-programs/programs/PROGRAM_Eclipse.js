...
function getAllPresets(funcClass, time, shape = "Warro") {
  const presets = [];
  _.each(funcClass.presets(), (preset, name) => {
    presets.push({
      duration: time * baseTime,
      program: programsByShape({ [shape]: [funcClass, preset] }, name)
    });
    console.log(`Loaded preset ${funcClass.name.green} ${name.yellow} from program presets.`);
  });
  if (presets.length === 0) {
    console.warn(`${funcClass} has 0 presets!`);
  }
  return presets;
}
...
const schedule = [
  { duration: 5 * baseTime, program: AllOff },
